...
<head>
    ...
    <title>Edit Career Path - MyCareer Admin</title>
    ...
    <div class="admin-container">
        ...
        <main class="admin-main">
            <header class="admin-header">
                <div class="header-title"><h1>Edit Career Path</h1></div>
            ...
            <section class="admin-content">
                ...
                <div class="form-card">
                    <form method="post" action="/admin/career-paths/edit/<?php echo htmlspecialchars($career['id'], ENT_QUOTES); ?>">
                        <div class="form-group">
                            <label>Name</label>
                            <input type="text" name="name" value="<?php echo htmlspecialchars($career['name'], ENT_QUOTES); ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Description</label>
                            <textarea name="description" rows="4"><?php echo htmlspecialchars($career['description'], ENT_QUOTES); ?></textarea>
                        </div>
                        <div class="form-group">
                            <label>More Info</label>
                            <textarea name="more_info" rows="4"><?php echo htmlspecialchars($career['more_info'] ?? '', ENT_QUOTES); ?></textarea>
                        </div>
                        <div class="form-group">
                            <label>Assign Interests</label>
                            <div class="interest-checkboxes">
                                <?php foreach ($availableInterests as $i): ?>
                                    <label class="checkbox-inline">
                                        <input type="checkbox" name="interests[]" value="<?php echo htmlspecialchars($i['id'], ENT_QUOTES); ?>" <?php echo in_array((int)$i['id'], $assigned) ? 'checked' : ''; ?>> <?php echo htmlspecialchars($i['name'], ENT_QUOTES); ?>
                                    </label>
                                <?php endforeach; ?>
                            </div>
                        </div>
